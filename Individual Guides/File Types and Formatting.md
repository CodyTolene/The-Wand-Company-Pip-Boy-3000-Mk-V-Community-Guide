...
#### Now we have our file its time to decide where to place it, You can either drop it into a pre-existing folder on your device like "misc" in which thr file will show in the STATS section of the DATA tab or you can create a new directory on your device and view the file with the file explorer app. For the sake of this guide ill be adding a short video to the misc section so it shows up in the UI.
